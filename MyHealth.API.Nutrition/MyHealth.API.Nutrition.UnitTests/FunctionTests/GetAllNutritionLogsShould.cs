...
namespace MyHealth.API.Nutrition.UnitTests.FunctionTests
{
    public class GetAllNutritionLogsShould
    {
        ...
        [Fact]
        public async Task ThrowBadRequestResultWhenWutritionEnvelopesAreNull()
        {
            // Arrange
            MemoryStream memoryStream = new MemoryStream();
            _mockHttpRequest.Setup(r => r.Body).Returns(memoryStream);
            _mockNutritionDbService.Setup(x => x.GetAllNutritionLogs()).Returns(Task.FromResult<List<mdl.NutritionEnvelope>>(null));
